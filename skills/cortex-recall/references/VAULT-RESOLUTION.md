...
After resolving the vault path, confirm it is a valid Cortex Forge vault: the path must contain both `wiki/` and `AGENTS.md`. If either is missing, stop and tell the user the resolved path does not look like a Cortex Forge vault — do not proceed to read `AGENTS.md` fields or attempt any vault operation.

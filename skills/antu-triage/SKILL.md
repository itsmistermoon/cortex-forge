...
2. **PLAYBOOK.md stale-entry pruning** — if `.hot/PLAYBOOK.md` exists, remove dated subsections (`### YYYY-MM-DD`) under `## Working context` older than 15 days, per `references/PLAYBOOK-FORMAT.md` (never touch `## Permanent`). No confirmation needed — this is a mechanical TTL rule, not a judgment call. If nothing qualifies, note "no stale entries found" rather than omitting the step. **Done when:** every Working-context entry older than 15 days is gone.

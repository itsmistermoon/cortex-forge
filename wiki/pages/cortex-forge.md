...
- Installable via `npx skills add itsmistermoon/cortex-forge` ([skills.sh](https://www.skills.sh/)), independent of the tarball/curl installer — every skill's scripts are co-located, so the skill is self-contained regardless of installer
...
- **State files written atomically, not in place.** `vault-report.json` (`cortex-prune.sh`) and `config.yml` (`install.sh`) used to write directly to their final path — a kill/crash mid-write could leave `vault-report.json` truncated, or worse, zero out `config.yml` and lose every registered vault. Both now write to a temp file in the same directory and `mv` atomically into place. Same audit investigated and ruled out as non-issues: paths with spaces (tested against `moon-vault`'s real spaced path), sqlite concurrent-write handling (Python's 5s default timeout plus a visible traceback is adequate for single-user use), and unbounded log growth (8KB after months of real use). Manual YAML parsing in 4 scripts remains a known low-priority risk — adopting a real parser was judged not worth a new dependency (`pyyaml`) for a small, machine-generated config schema.
- **`cortex-forge-setup` detects a missing/incomplete tarball runtime instead of creating broken symlinks.** Step 4 used to assume `~/.cortex-forge/skills/` (the tarball runtime) was complete before symlinking from it — if it was missing or partial, skills silently got broken symlinks with no explanation. New step 4-pre checks all 6 skills first; if any are missing, it tells the user and recommends `npx skills add itsmistermoon/cortex-forge --all -g -y` (doesn't depend on the tarball runtime at all) ahead of re-running the curl installer.

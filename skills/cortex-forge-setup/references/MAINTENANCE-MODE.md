...
Reference for `cortex-forge-setup` step 2. Read only when step 2 determines the vault is already registered — never loaded for a new-vault run. Covers the menu shown to the user and how steps 3–6 map to it, including the differences from the new-vault flow where any exist (steps 3, 3a, and 5).

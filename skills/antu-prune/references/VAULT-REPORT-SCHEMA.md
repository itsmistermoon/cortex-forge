...
Reference for `antu-prune` (step 5). Read when adding fields or checking what the Layer 1 script persists.

`{vault}/wiki/meta/vault-report.json` is the session-startup health signal read in `AGENTS.md`. Gitignored — a local artifact, not versioned content. Written by `scripts/antu-prune.sh` on every Layer 1 run — the agent never writes it directly.

...
Reference for `wiki-ingest`. Read when the first argument starts with `--research`, discovering and ingesting sources on the web instead of processing a single given URL or file.

```text
/wiki-ingest --research "embeddings for second brains" [--rounds N]
```

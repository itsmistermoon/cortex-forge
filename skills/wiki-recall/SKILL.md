...
1. **Resolve vault** — per `references/VAULT-RESOLUTION.md`. If the first argument matches a registered vault name (e.g., `/wiki-recall second-brain <query>`), use that vault; treat the remaining text as the query.
...
4. **Offer to persist, rarely** — only when the answer combines two or more existing pages into an insight not written down anywhere in the vault, or fills a real gap the wiki had no page for, end the response with one localized line equivalent to: "This isn't written anywhere in the vault yet — want me to save it? (`/wiki-imprint`)" (translated to the conversation's language, per this skill's own opening rule). Skip this for anything answerable by pointing at a single existing page — the offer must stay rare, not a footer on every response. Persisting itself is not this skill's job: acceptance (now or in a later message) invokes `/wiki-imprint`, treating this answer as the source content.

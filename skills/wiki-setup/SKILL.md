...
- **`scripts/index.py`** — Builds/refreshes `.hot/db/vault.db` when semantic search is enabled (step 5); also copied to `~/.cortex-forge/bin/` for the post-commit reindex hook (step 5a)
- **`scripts/embeddings.py`** — Shared embedding backend, imported by `index.py`; not invoked directly
- **`scripts/reindex-post-commit.sh`** — Copied to `~/.cortex-forge/bin/hooks/` and wired into `{vault}/.git/hooks/post-commit` (step 5a)
...
3c. **Offer stale-cache warning threshold (opt-in, global setting)** — this is a single global value in `~/.cortex-forge/config.yml` (top-level, like `imprint_triage`), not per-vault. Read the config first:
    - **If `hot_cache_stale_days:` (the old key name, pre-ADR-0003) is present and `playbook_stale_days:` is not** → migrate: read the old value, write it as `playbook_stale_days: N`, remove `hot_cache_stale_days:`, and note in the final summary that the key was renamed (value preserved) — never ask the user to re-enter a value that's already on disk.
    ...
    - **If neither is set** → ask: "Warn if this vault's memory hasn't been touched in N days? (default: 15, 0 to disable)"
...
4. **Verify global skills are installed** — check each of the 7 skills (`hot-handoff`, `wiki-setup`, `wiki-recall`, `wiki-ingest`, `wiki-imprint`, `wiki-prune`, `hot-triage`) individually for presence under `~/.agents/skills/` — do not assume presence just because this skill is running.
...
- Stale-cache threshold (if set/changed/migrated): `playbook_stale_days` value, or "default (15)" if left unset — note if it was migrated from the old `hot_cache_stale_days` key

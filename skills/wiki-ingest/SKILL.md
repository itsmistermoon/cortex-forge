...
1. **Resolve vault** — per `references/VAULT-RESOLUTION.md`, then its `locale:` per `references/LOCALE-RESOLUTION.md`. If the first argument matches a registered vault name (e.g., `/wiki-ingest second-brain <url>`), use that vault and treat the remaining argument as the URL or file path.

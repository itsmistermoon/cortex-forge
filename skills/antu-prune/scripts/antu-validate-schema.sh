#!/bin/bash
# antu-validate-schema.sh — validate frontmatter completeness against schema v0.3
# Usage: antu-validate-schema.sh [vault-path] [findings-file]
#   vault-path     defaults to git root or pwd
#   findings-file  if given, findings are APPENDED there (same format as antu-prune.sh)

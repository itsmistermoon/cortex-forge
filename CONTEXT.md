...
- **`hot-triage`** — Antu-only, on-demand skill for `.hot/` hygiene: recovering pending/fragile-context items buried in `HISTORY.md` after a cross-suite write, re-evaluating `### Pending`/`### Active decisions` validity, retrospective `PLAYBOOK.md` candidate review, and deeper `HISTORY.md` cleanup. Mirrors `wiki-prune`'s pattern (separate hygiene skill vs. folding into a skill that runs every session). No Kuyen equivalent.

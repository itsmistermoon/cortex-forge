...
|--------|---------|------------------|-------------------|
| `cortex-prune.sh` | `skills/cortex-prune/` | `/cortex-prune` (directo) | El agente lo resuelve como hermano de `SKILL.md` — misma carpeta de donde leyó las instrucciones |
| `cortex-sanitize.sh` | `skills/cortex-assimilate/` | `/cortex-assimilate` paso 4a | Igual — hermano de `SKILL.md` |
| `cortex-index.py`, `cortex-search.py`, `embeddings.py` | `skills/cortex-forge-setup/` | `/cortex-forge-setup` los copia a `{vault}/.cortex/db/`; desde ahí `/cortex-recall` y `/cortex-assimilate` (reindex) los invocan como copias locales del vault | `embeddings.py` se resuelve como hermano de `cortex-search.py`/`cortex-index.py` en cualquiera de las dos ubicaciones (skill o vault) |
...
**Flujo del reindex:** el script detecta cuántos archivos `wiki/` cambiaron en el commit (`git diff-tree`), y solo entonces invoca `{vault}/.cortex/db/cortex-index.py` sobre la raíz del vault (la copia local del vault, no la de la skill). Si la DB no existe o no hay archivos wiki modificados, sale silenciosamente (`exit 0`). El bloque usa `|| true` para ser fail-open — nunca bloquea un commit.
